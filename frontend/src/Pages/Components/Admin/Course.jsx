...
const Course = () => {
  ...
  useEffect(() => {
    console.log("Fetched Data from useParams:", data); // Debugging statement

    if (!data) {
      console.error("Error: 'data' parameter is undefined. Check your route.");
      return;
    }
    ...
    const fetchCourseRequests = async () => {
      try {
        const response = await axios.get(`/api/admin/${data}/approve/course`);
        console.log("Course Requests Data:", response.data.data);
        setCourseReq(response.data.data);
      } catch (error) {
        console.error('Error fetching course requests:', error);
      }
  ...
  return (
    <div className='h-[100vh]'>
      {/* Navbar */}
      <nav className="h-16 w-full bg-[#042439] flex justify-between items-center px-4">
        <div className="flex items-center gap-4">
          <h1 onClick={() => navigate(`/admin/${data}`)} className="text-lg text-blue-700 font-bold font-mono">
            ◀ Back
          </h1>
        </div>
        <div className="flex items-center">
      ...
      {courseReq.length > 0 ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            {courseReq.map((req, index) => (
              <div key={index} className="bg-gray-800 p-4 rounded-md shadow-lg">
                <h2 className="text-lg text-yellow-500 font-bold">{req.coursename.toUpperCase()}</h2>
                <p className="text-yellow-700 font-semibold">{req.description}</p>
                <p className="text-yellow-300">Enrolled Teacher: {req.enrolledteacher.Firstname} {req.enrolledteacher.Lastname}</p>
                <div className='flex flex-row gap-3 mt-2'>
                  <button className='text-white bg-green-500 px-3 py-1' onClick={() => handleAccept(req._id, { Email: req.enrolledteacher.Email, enrolledteacher: req.enrolledteacher.Firstname })}>Approve</button>
                  <button className='text-white bg-red-500 px-3 py-1' onClick={() => handleReject(req._id, { Email: req.enrolledteacher.Email, enrolledteacher: req.enrolledteacher.Firstname })}>Reject</button>
                </div>
              </div>
            ))}
          </div>
        ...
        <p className="text-center text-white mt-5">No course requests available.</p>
      )}
    </div>
  );
};
