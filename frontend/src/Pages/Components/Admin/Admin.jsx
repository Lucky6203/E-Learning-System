...
import { NavLink, useNavigate, useParams } from "react-router-dom";
import logo from "../../Images/logo.svg";
import Course from "./Course"; // Ensure it's being used properly
import axios from "axios";
...
const Admin = () => {
  ...
  useEffect(() => {
    const getAllMsg = async () => {
      try {
        const response = await axios.get("http://localhost:9000/api/admin/messages/all");
  ...
  const Approval = async (ID, type, approve) => {
    try {
      ...
      await axios.post(`/api/admin/${adminID}/approve/${type}/${ID}`, data);
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        const response = await axios.post(`/api/admin/${data}/approve`);
  ...
  return (
    <div className="h-[100vh]">
      ...
      <div className="flex items-start justify-center gap-20">
        ...
        <div className="rounded-md">
          <h4 className="text-white bg-red-500 p-4 w-40">Rejected Request</h4>
          {[...TeacherData, ...StudentData].map(
            (user) =>
              user.Isapproved === "rejected" && (
                <div key={user._id} onClick={() => docDetails(user.role, user._id)} className="flex justify-around items-center mt-8 p-8 bg-blue-gray-600 rounded-md cursor-pointer">
                  <h1 className="text-[24px] text-1xl text-white mr-3">
                    {user.Firstname + " " + user.Lastname}
                  </h1>
                  <p>Msg: <span>{user.Remarks}</span></p>
                </div>
              )
          )}
        </div>
