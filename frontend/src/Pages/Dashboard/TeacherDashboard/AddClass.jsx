...
function AddClass({ onClose }) {
  ...
  function setToMidnight(dateTimeString) {
    ...
    let totalMinutes = (hours * 60) + minutes;
    date.setUTCHours(0, 0, 0, 0);
    return [totalMinutes, date.toISOString()];
  }
  ...
  // Update allowedDays when CourseId changes
  useEffect(() => {
    if (courses.length && CourseId) {
      const selectedCourse = courses.find(c => c._id === CourseId);
      setAllowedDays(selectedCourse?.schedule || []);
    }
  }, [CourseId, courses]);
  ...
  const addCourses = async () => {
    const currentDate = new Date();
    const givenDate = new Date(date);
    const [timing, formattedDate] = setToMidnight(date);
